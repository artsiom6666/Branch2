({
	getSettingsHelper: function(component) {
		...
		var action = component.get('c.getMinFraudSettingsApex');
		action.setCallback(this, function(response) {
			...
			if (component.isValid() && state === 'SUCCESS') {
				...
				var settings = {};
				settings.apiKey = (result['MinFraud.ApiKey']) ? result['MinFraud.ApiKey'] : '';
				settings.firstRating = (result['MinFraud.RatingFirst']) ? +result['MinFraud.RatingFirst'] : 0;
				settings.secondRating = (result['MinFraud.RatingSecond']) ? +result['MinFraud.RatingSecond'] : 0;
