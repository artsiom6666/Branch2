({
	getSettingsHelper: function(component) {
		...
		var action = component.get('c.getCmSettingsApex');
		action.setCallback(this, function(response) {
			...
			if (component.isValid() && state === 'SUCCESS') {
                ...
                //console.log(result);
                var settings = {};
                settings.selectedStateCampaignMember = (result['Batch.CampaignMemberStatus']) ? result['Batch.CampaignMemberStatus'] : 'Off';
                settings.selectedExecuteAtCampaignMember = (result['Batch.CampaignMemberTime']) ? result['Batch.CampaignMemberTime'] : '1';
                settings.selectedExecuteAtCampaignMemberDay= (result['Batch.CampaignMemberDay']) ? result['Batch.CampaignMemberDay'] : '0';
		...
		var getTime = component.get('c.getCmTimeScheduleJobs');
