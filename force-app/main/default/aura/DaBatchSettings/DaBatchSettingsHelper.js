({
	getSettingsHelper: function(component) {
		...
		var action = component.get('c.getDaSettingsApex');
		action.setCallback(this, function(response) {
			...
			if (component.isValid() && state === 'SUCCESS') {
                ...
                var settings = {};
                settings.selectedStateDa = (result['Batch.DataAppendState']) ? result['Batch.DataAppendState'] : 'Off';
                settings.selectedExecuteAtDa = (result['Batch.DataAppendTime']) ? result['Batch.DataAppendTime'] : '1';
		...
		var getTime = component.get('c.getDaTimeScheduleJobs');
