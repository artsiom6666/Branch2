({
	getSettingsHelper: function(component) {
		...
		var getSettingsApex = component.get('c.getApprovedSettingsApex');

		var getUsersApex = component.get('c.getApprovedUsersApex');
		...
		getSettingsApex.setCallback(this, function(response) {
			...
			if (component.isValid() && state === 'SUCCESS') {
                ...
                //console.log(result);
                var settings = {};
                settings.DALimit = (result.DALimit) ? +result.DALimit : 0;
